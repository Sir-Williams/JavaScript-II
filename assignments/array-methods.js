...
// Problem 1
//Display all the runners last names with lower case letters
let noCaps = [];

runners.map((runner, index) => {
    noCaps[index] = runner.last_name.toLowerCase();
});

console.log(noCaps);

// Problem 2
//Count how many donations are above 100 dollars
let onehundredOrMore = [];

onehundredOrMore = runners.reduce((totalValue, runner) => {
        return totalValue + (runner.donation >= 100);
}, 0)

console.log(onehundredOrMore);

// Problem 3
//Show each person's email so they can be congratulated

let congratulations = [];

runners.forEach(runner => {
    congratulations.push(`${runner.email}`)
});
